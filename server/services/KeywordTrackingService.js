...
export async function keywordTracking(tracking){
    try{
        ...
        if(result.success){
              ...
              tracking.currentPostion = result.data.position;
              tracking.currentPage = result.data.page;
              tracking.competitors = result.data.competitors;
              tracking.lastChecked = new Date();
              tracking.status = "completed";
              //Update status 
              tracking.positionChange = prev && result.data.postion ? prev - result.data.position : 0;
              if(result.data.postion && (!tracking.bestPosition ||  result.data.postion < tracking.bestPosition)){
                tracking.bestPosition = result.data.postion;
              }
                  //Update history
                  const historyEntry = {
                    date:today,
                    postion: result.data.postion,
